...
Для проверки работы сервиса в GitHub используется отдельный репозиторий:
[`vlgUseless/megafix-test-repo`](https://github.com/vlgUseless/megafix-test-repo).

В нём созданы тестовые issue/PR для проверки end-to-end сценариев:
...
- В текущем репозитории около **90** тестов на `pytest` (в основном unit/contract, плюс интеграционные сценарии).
- Базовый запуск:
...
megafix review-pr --pr-url https://github.com/<owner>/<repo>/pull/<id> --publish
# или эквивалентно:
...
python -m megafix.interfaces.cli review-pr --pr-url https://github.com/<owner>/<repo>/pull/<id> --publish
```
